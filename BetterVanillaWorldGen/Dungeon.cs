...
namespace AdvancedWorldGen.BetterVanillaWorldGen
{
	public class Dungeon
	{
		...
		public static bool PlaceSandTrap(int i, int j)
		{
			...
			if (!Main.tileSolid[Main.tile[i, k].type] || Main.tile[i, k].IsHalfBlock ||
			    Main.tile[i, k].Slope == SlopeType.SlopeDownLeft || Main.tile[i, k].Slope == SlopeType.SlopeDownRight)
				return false;

			if (Main.tile[i, k].type != 53 && Main.tile[i, k].type != 397 && Main.tile[i, k].type != 396 ||
			    Main.tile[i, k].wall != 216 && Main.tile[i, k].wall != 187)
				return false;
			...
			for (int num7 = k + 1; num7 <= k + 3; num7++)
				if (!Main.tile[n, num7].IsActive || !Main.tileSolid[Main.tile[n, num7].type])
					return false;
			...
			{
				if (Main.tile[num11, num10].IsActive && Main.tileSolid[Main.tile[num11, num10].type])
					return false;

				for (int num12 = num4 - num6; num12 <= num4; num12++)
					if (Main.tile[num11, num12].IsActive)
					{
			...
			float num13 = (num5 * 2 + 1) * (num6 + 1) * 0.75f;
			if (num9 < num13)
				return false;
			...
			for (int num15 = num4 - num6; num15 <= num4; num15++)
			{
				...
				if (num15 == num4)
				{
					Main.tile[num14, num15].Slope = SlopeType.Solid;
					Main.tile[num14, num15].IsHalfBlock = false;
					if (!flag)
					{
						Main.tile[num14, num15].IsActive = true;
				...
				else if (num15 == num4 - num6)
				{
					...
					Main.tile[num14, num15].IsActive = true;
					if (flag && Main.tile[num14, num15 - 1].IsActive &&
					    Main.tileSolid[Main.tile[num14, num15 - 1].type])
			...
			for (int num16 = (int) (num4 - num6 * 0.666f);
				(double) num16 <= (double) num4 - (double) (float) num6 * 0.333;
				num16++)
			{
				if (num16 < num4 - num6 * 0.4f)
				{
					if (Main.tile[i - num5 - 2, num16].Slope == SlopeType.SlopeUpLeft ||
					    Main.tile[i - num5 - 2, num16].Slope == SlopeType.SlopeUpRight)
						Main.tile[i - num5 - 2, num16].Slope = SlopeType.Solid;
				}
				else if (num16 > num4 - num6 * 0.6f)
				{
					if (Main.tile[i - num5 - 2, num16].Slope == SlopeType.SlopeDownLeft ||
					    Main.tile[i - num5 - 2, num16].Slope == SlopeType.SlopeDownRight)
						Main.tile[i - num5 - 2, num16].Slope = SlopeType.Solid;

					Main.tile[i - num5 - 2, num16].IsHalfBlock = false;
				}
				else
				{
					Main.tile[i - num5 - 2, num16].IsHalfBlock = false;
				...
				if (!Main.tile[i - num5 - 2, num16].IsActive || !Main.tileSolid[Main.tile[i - num5 - 2, num16].type])
				{
					Main.tile[i - num5 - 2, num16].IsActive = true;
			...
			{
				Main.tile[i - num5 - 2, num17].Slope = SlopeType.Solid;
				Main.tile[i - num5 - 2, num17].IsHalfBlock = false;
				Main.tile[i - num5 - 1, num17].Slope = SlopeType.Solid;
				Main.tile[i - num5 - 1, num17].IsHalfBlock = false;
				Main.tile[i - num5 + 1, num17].Slope = SlopeType.Solid;
				Main.tile[i - num5 + 1, num17].IsHalfBlock = false;
				Main.tile[i - num5 + 2, num17].Slope = SlopeType.Solid;
				Main.tile[i - num5 + 2, num17].IsHalfBlock = false;
			...
			{
				int num19 = k - num6 - 1;
				if (Main.tile[num18, num19].Slope == SlopeType.SlopeUpLeft ||
				    Main.tile[num18, num19].Slope == SlopeType.SlopeUpRight)
					Main.tile[num18, num19].Slope = SlopeType.Solid;

				Main.tile[num18, num19].IsHalfBlock = false;
			}
			...
			{
				int num21 = k;
				if (num20 < i - num5 * 0.8 || num20 > i + num5 * 0.8)
					num21 = k - 3;
				else if (num20 < i - num5 * 0.6 || num20 > i + num5 * 0.6)
					num21 = k - 2;
				else if (num20 < i - num5 * 0.4 || num20 > i + num5 * 0.4)
					num21 = k - 1;

				for (int num22 = num4; num22 <= k; num22++)
				{
					if (num20 == i && num22 <= k)
						Main.tile[i, num22].RedWire = true;

					if (Main.tile[num20, num22].IsActive && Main.tileSolid[Main.tile[num20, num22].type])
					{
						if (num22 < num4 + num - 4)
						{
							Main.tile[num20, num22].HasActuator = true;
							Main.tile[num20, num22].RedWire = true;
						}
						else if (num22 < num21)
						{
