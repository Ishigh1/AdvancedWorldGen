...
public class Spooky : ModSystem
{
	...
	public override void Load()
	{
		if (ModLoader.TryGetMod("Spooky", out Mod mod) &&
		    mod.TryGetMethod("Spooky.Content.Generation.SpookyHell", "ClearArea", BindingFlags.Public | BindingFlags.Instance,
	...
	private static void AvoidRight(ILContext ilContext) //Will have to remove when it is implemented
	{
		...
		cursor.GotoNext(MoveType.Before, instruction => instruction.OpCode == OpCodes.Ble_S && instruction.Next.MatchLdloc(0));
		cursor.Remove();
