...
public class EditableText<T> : UIElement, IEditableText where T : IConvertible, IComparable
{
	...
	protected override void DrawSelf(SpriteBatch spriteBatch)
	{
		...
		if (CurrentContent != null)
		{
			...
			if (Main.inputTextEnter) Unfocus();
			else //Move cursor
			{
				if (Main.inputText.IsKeyDown(Microsoft.Xna.Framework.Input.Keys.Left))
				{
					if (FrameNumber++ % 10 == 0)
						Shift = Math.Min(CurrentContent.Length, Shift + 1);
				}
				else if (Main.inputText.IsKeyDown(Microsoft.Xna.Framework.Input.Keys.Right))
				{
					if (FrameNumber++ % 10 == 0)
